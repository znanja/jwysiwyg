...
task :lint => [:prepare, :package] do
  opts = fetch_options
  system("#{ENV['JSLINT']} -process #{File.join(distpath, opts['output']['default'])}")
end
...
task :prepare do
  ...
  ENV['JSLINT']          = "jsl"
  puts "Creating distribution path... #{distpath}"
...
def distpath
  opts = fetch_options
  File.expand_path(File.join(opts['paths']['dist'], opts['version']))
end

def fetch_options
  @fetched_options ||= YAML::load(File.open(File.expand_path('.', "build.yml")))
end
